...
function mergePingResults(from, to) {
  if (!from || !to) {
    return from;
  }

  to.rtts = from.rtts.concat(to.rtts);
  return to;
}

function newHop(hopNumber) {
  return {
    'ip': null,
    'hostname': null,
    'number': hopNumber,
...
Mtr.prototype._run = function(target) {
  ...
  child.on('exit', function(code) {
    ...
    if (code === 0) {
      ...
      result.forEach(function(hop, i) {
        if (!hop) {
          emitter.emit('hop', newHop(i));
          return;
        }
...
Mtr.prototype._parseResult = function(output) {
  var lines, line, i, j, split, type, hopNumber, data, value, previousResult, results,
      result, which, maxHopNumber = 0, item, related = {};
  ...
  // Second pass combine the duplicates
  for (i = results.length - 1; i > 0; i--) {
    ...
    if (result.ip === previousResult.ip) {
      mergePingResults(result, previousResult);
      results.splice(-1, 1);
    } else {
      break;
    }
